...
```sh
singularity exec docker://nciccbr/carlisle_r:v2 R -e "readr::write_csv(tibble::as_tibble(installed.packages()), 'r-packages.csv')"
```

This command runs R code in the container to write the installed packages to a TSV file called `r-packages.csv`.
